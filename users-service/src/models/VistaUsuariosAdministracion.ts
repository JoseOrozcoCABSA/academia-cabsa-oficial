/**
 * @file Modelo `VistaUsuariosAdministracion` — tabla `usuarios_vista_administracion`.
 *
 * 32 columnas, 25 obligatorias.
 * Clave primaria: `id`.
 *
 ...
 * - `grupos`: TEXT
 * - `total_grupos`: BIGINT  · NOT NULL
 * - `membresia_activa`: BIGINT
 * - `membresias`: TEXT
 * - `fecha_limite_membresia`: DATE
 *
...
/**
 * Modelo Sequelize de `usuarios_vista_administracion`.
 *
 * La clase queda vacía a propósito: las columnas y las opciones se
 * declaran en `VistaUsuariosAdministracion.init()`, justo debajo. Cualquier consulta pasa
 * por el repositorio genérico, no por este modelo directamente.
 */
export default class VistaUsuariosAdministracion extends Model {}

VistaUsuariosAdministracion.init({
  ...
  "grupos": { type: DataTypes.TEXT, allowNull: true },
  "total_grupos": { type: DataTypes.BIGINT, allowNull: false },
  "membresia_activa": { type: DataTypes.BIGINT, allowNull: true },
  "membresias": { type: DataTypes.TEXT, allowNull: true },
  "fecha_limite_membresia": { type: DataTypes.DATE, allowNull: true },
}, {
  sequelize: database,
  modelName: 'VistaUsuariosAdministracion',
  tableName: "usuarios_vista_administracion",
  timestamps: false,
