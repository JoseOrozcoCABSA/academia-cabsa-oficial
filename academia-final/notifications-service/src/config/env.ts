...
const env: Readonly<Environment> = Object.freeze({
  ...
  supportStoragePath: process.env.SUPPORT_STORAGE_PATH?.trim() || 'src/storage/support',
  smtp: {
    host: process.env.SMTP_HOST ?? 'smtp.gmail.com',
    port: numberValue('SMTP_PORT', 465),
    secure: (process.env.SMTP_SECURE ?? 'true').toLowerCase() === 'true',
    user: requiredValue('SMTP_USER'),
    password: requiredValue('SMTP_PASSWORD', 16).replace(/\s+/g, ''),
    fromName: process.env.MAIL_FROM_NAME?.trim() || 'Academia CABSA',
    fromAddress: process.env.MAIL_FROM_ADDRESS?.trim() || requiredValue('SMTP_USER'),
  },
  database: {
